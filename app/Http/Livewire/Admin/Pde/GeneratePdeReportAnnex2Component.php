...
class GeneratePdeReportAnnex2Component extends Component
{
    public function printPDF($datefrom, $dateto)
    {
        ...
        $templatePath = public_path() . '/pdecertificate/Annex 2 front page.pdf';
        ...
        $pdf->SetTitle('Generated PDE Annex 2');
        ...
        for ($i = 1; $i <= $pageCount; $i++) {
            ...
            $no2 = 1;
                ...
                $pdf->Cell(9, 5.5, $no.".", 0, 0, 'L', 0, '', 0);
                //NAME
                $pdf->SetFont($gothic, '', 8, '', true);
                $pdf->Cell(54.3, 5.5, strtoupper($firstname . ' ' . substr($middlename, 0, 1) . '. ' . $surname), 0, 0, 'L', 0, '', 0);

                //PASSPORT NUMBER
                $pdf->SetFont($gothic, '', 8, '', true);
                $pdf->Cell(42.8, 5.5, strtoupper($passportno), 0, 0, 'L', 0, '', 0);
                //CERTIFICATE NUMBER
                $pdf->SetFont($gothic, '', 8, '', true);
                $pdf->Cell(42.5, 5.5, $certificatenumber, 0, 0, 'L', 0, '', 0);
                //CERTIFICATE ISSUANCE DATE
                $pdf->SetFont($gothic, '', 8, '', true);
                $pdf->Cell(38, 5.5, date_format(date_create($certdateprinted), "d F Y"), 0, 1, 'L', 0, '', 0);

                if ($no == 33) {
                    $pdf->AddPage('P', 'A4');
                    //Annex 1 body page
                    $formpath = "pdecertificate/Annex-2-P2.jpg";
    ...
    {
        return view('livewire.admin.pde.generate-pde-report-annex2-component');
    }
}
